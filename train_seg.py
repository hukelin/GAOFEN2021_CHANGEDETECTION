...
class Trainer:
    def __init__(self, args):
        self.args = args
        # 加载数据
        trainset = Segmentation(root=args.train_root, mode="train")
        valset = Segmentation(root=args.val_root, mode="val")
        ...
        self.model = FarSeg(backbone=args.backbone, classes=2)
        # *******************************模型选择*************************************
        # 加载预训练模型
    ...
    def validation(self):
        ...
        with torch.no_grad():
            for i, (img, label, id) in enumerate(tbar):
                ...
                # 模型输出
                out_seg = self.model(img)
                loss_seg1 = self.criterion_seg(
                    out_seg.squeeze(), mask_seg.long())
                loss_seg2 = self.criterion(
                    out_seg.squeeze(), mask_seg.long(), self.iters, self.total_iters)
                loss = loss_seg1+loss_seg2
                loss = loss.mean()
