...
class FarSeg(Module):
    ...
    * https://arxiv.org/pdf/2011.09766.pdf
    ...
    def __init__(
        ...
        self.backbone = HRNet(backbone, backbone_pretrained)
        # max_channels =
        # self.fpn = FPN(
        ...
        self.decoder = _LightWeightDecoder2(450, 225, classes)
    ...
    def forward(self, x: Tensor, tta=False) -> Tensor:
        if not tta:
           ...
           # 原图像输出
            ...
            # # 换轴再翻转
            x = origin_x.transpose(2, 3)
            cur_out = self.base_forward(x)
            out += cur_out.transpose(2, 3)
    ...
    * https://arxiv.org/pdf/2011.09766.pdf
    ...
    def __init__(
        ...
        Args:
            ...
            backbone_pretrained: whether to use pretrained weight for backbone
        """
        super(FarSeg_Res, self).__init__()  # type: ignore[no-untyped-call]
    ...
    * https://arxiv.org/pdf/2011.09766.pdf
    ...
    def __init__(
        ...
        Args:
            ...
            backbone_pretrained: whether to use pretrained weight for backbone
        """
        super(FarSeg_CD_Res, self).__init__()  # type: ignore[no-untyped-call]
        if backbone in ["resnet18", "resnet34"]:
            max_channels = 512
        elif backbone in ["resnet50", "resnet101"]:
            max_channels = 2048
...
#         features_bin = torch.abs(features2-features1)
#         out_bin = self.decoder(features_bin)
#         out = torch.softmax(out_bin, dim=1)
#         return out
...
class FarSeg_CD(Module):
    ...
    * https://arxiv.org/pdf/2011.09766.pdf
    ...
    def base_forward(self, x1: Tensor, x2: Tensor) -> Tensor:
        ...
        features_bin = self.extract_features(x)
        out_bin = self.decoder(features_bin)
        out = torch.softmax(out_bin, dim=1)
        return out
...
class _LightWeightDecoder2(Module):
    ...
    def forward(self, features: List[Tensor]) -> Tensor:
        """Forward pass of the model."""
        decoder_feat = self.blocks(features)
        out = self.classifier(decoder_feat)
        return out
