...
class SceneRelation(nn.Module):
    ...
    def forward(self, scene_feature, features: list):
        content_feats = [c_en(p_feat) for c_en, p_feat in zip(
            self.content_encoders, features)]
        if self.scale_aware_proj:
            ...
            relations = [self.normalizer(
                (scene_feat * cf).sum(dim=1, keepdim=True)) for cf in content_feats]
    ...
    def __init__(self,
        ...
        for in_feat_os in in_feat_output_strides:
            ...
            self.blocks.append(nn.Sequential(*[
                nn.Sequential(
                    nn.Conv2d(in_channels if idx == 0 else out_channels,
...
class FarSeg(CVModule):
    def __init__(self, config):
        ...
        self.register_buffer(
            'buffer_step', torch.zeros((), dtype=torch.float32))
        ...
        self.cls_pred_conv = nn.Conv2d(
            self.config.decoder.out_channels, self.config.num_classes, 1)
        ...
        if 'annealing_softmax_focalloss' in self.config:
            print('loss type: {}'.format(
                self.config.annealing_softmax_focalloss.annealing_type))
    ...
    def forward(self, x, y=None):
        ...
        if self.training:
            ...
            loss_dict = dict()
            self.buffer_step += 1
            cls_loss_v = self.config.loss.cls_weight * \
                self.cls_loss(cls_pred, cls_true)
            loss_dict['cls_loss'] = cls_loss_v

            mem = torch.cuda.max_memory_allocated() // 1024 // 1024
            loss_dict['mem'] = torch.from_numpy(
                np.array([mem], dtype=np.float32)).to(self.device)
            return loss_dict
